...
<article>
	...
	<?php } ?>
	<?php if( get_theme_mod( 'stock_photos_feature_image',true) != '') { ?>
	...
	<?php if( get_theme_mod( 'stock_photos_tags',true) != '') { ?>
	...
	<?php
	wp_link_pages( array(
		'before'      => '<div class="page-links"><span class="page-links-title">' . __( 'Pages:', 'stock-photos' ) . '</span>',
		'after'       => '</div>',
		'link_before' => '<span>',
		'link_after'  => '</span>',
		'pagelink'    => '<span class="screen-reader-text">' . __( 'Page', 'stock-photos' ) . ' </span>%',
		'separator'   => '<span class="screen-reader-text">, </span>',
	) );

	if( get_theme_mod( 'stock_photos_comment',true) != '') {
	...
	if ( is_singular( 'attachment' ) ) {
		...
		the_post_navigation( array(
			'prev_text' => _x( '<span class="meta-nav">Published in</span><span class="post-title m-3">%title</span>', 'Parent post link', 'stock-photos' ),
		) );
	} elseif ( is_singular( 'post' ) ) {
		if( get_theme_mod( 'stock_photos_nav_links',true) != '') {
			// Previous/next post navigation.
			the_post_navigation( array(
				'next_text' => '<span class="meta-nav" aria-hidden="true">' . esc_html(get_theme_mod('stock_photos_next_text',__( 'Next Post', 'stock-photos' ))) . '<i class="fas fa-chevron-right"></i></span> ' .
					'<span class="screen-reader-text">' . __( 'Next Post', 'stock-photos' ) . '</span> ' .
					'',
				'prev_text' => '<span class="meta-nav" aria-hidden="true"><i class="fas fa-chevron-left"></i>' . esc_html(get_theme_mod('stock_photos_prev_text',__( 'Previous Post', 'stock-photos' ))) . '</span> ' .
					'<span class="screen-reader-text">' . __( 'Previous Post', 'stock-photos' ) . '</span> ' .
					'',
			) );
		}
	}?>
...
<?php if (get_theme_mod('stock_photos_related_posts',true) != '') {
